...
class AESpec extends FlatSpec with AE {
  ...
  "`parseMul2`" should "parse multiplication" in {
    assert(parseMul2("multiply 1 by 1") ==
      Some((Mul(Num(1), Num(1)), "")))

    assert(parseMul2("multiply 1234 by 5678") ==
      Some((Mul(Num(1234), Num(5678)), "")))

    assert(parseMul2("multiply 1 by 2 by 3 by 4") ==
      Some((Mul(Num(1), Num(2)), " by 3 by 4")))

    assert(parseMul2("add 1 to 1") == None)
    assert(parseMul2("1 * 1") == None)
    assert(parseMul2("multiply x to y") == None)
  }  
